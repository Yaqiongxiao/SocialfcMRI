...
flm_FC <- function(dat, mean_FC, term) {
	...
	if (term == "Age") {
		fl <- as.formula(paste0(mean_FC, "~ social*age + mental + gender + mean_FD+IQ"))
		fl2 <- as.formula(paste0(mean_FC, "~ age + mental + gender + mean_FD+IQ"))
	} else if (term == "RT") {
		fl <- as.formula(paste0(mean_FC, "~ social*RT + mental + gender + mean_FD+IQ"))
